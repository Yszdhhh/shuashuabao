...
# The operator key hook is generated by build_release.ps1 and compiled into
# the EXE as a script. No mutable JSON file becomes a runtime trust anchor.
manifest_trust_hook = os.environ.get("SHUABAO_BUILD_MANIFEST_TRUST_HOOK", "")
if not manifest_trust_hook or not Path(manifest_trust_hook).is_file():
    raise RuntimeError("BLOCKED: missing build-time operator manifest trust hook")
...
a = Analysis(
    ...
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[str(PROJECT_ROOT / "packaging" / "pyi_rth_pyside6_path.py"), manifest_trust_hook],
