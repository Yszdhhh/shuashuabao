...
def _capture_print_window(target: WindowTarget) -> Frame | None:
    ...
    try:
        ...
        if rgb.shape[2] >= 4:
            rgb = rgb[:, :, :3]
        bgr = rgb[:, :, ::-1].copy()
        # 尺寸自证：ImageGrab 抓取窗口位图尺寸 vs 期望 client 尺寸。
        # 不一致 → 无效帧 + 诊断 error；不裁剪、不拉伸、不伪装坐标。
        ...
        if (cap_w, cap_h) != (expect_w, expect_h):
            err = (
                f"[capture] 尺寸自证：ImageGrab 捕获 {cap_w}x{cap_h} vs "
                f"client {expect_w}x{expect_h}，不一致（不裁剪，返回无效帧）"
            )
            print(err)
            return Frame(
                bgr=np.zeros((0, 0, 3), dtype=np.uint8),
                left=target.client_left if target.client_left > 0 else target.left,
                top=target.client_top if target.client_top > 0 else target.top,
