...
class Mediator:
    ...
    def _public_bag_source(self, frame: Frame, layout: BagLayout) -> dict | None:
        ...
        Drain the personal grid first (a just-stashed pill sits there), then
        the item bar. A slot that is neither clearly empty nor occupied is
        skipped, not guessed.
        """
        for row, col in layout.public_slots():
    ...
    def _tick_lobby_hitch(
        ...
        if in_room and not self._hitch_re_search:
            ...
            if seat_decision == "leave_host_not_floor_one":
