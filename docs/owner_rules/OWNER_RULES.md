...
对账基准：round4 头 8d81b4a9（2026-09-26）。保留 #58 的羁绊禁拿名单实现（默认空、看板可配、所有拿卡路径排除），并吸收 #64 的有效容量保护。
...
| 3.4 | 羁绊禁拿名单默认空、看板可配；命中卡常规、模板直拿和刷新耗尽兜底都不得拿。禁字法前置、三国四选三等独立条件门继续保留 | Owner 2026-09-26 03:33（#58） | `bond_banned`、`bond_banned` policy gate | `test_bond_ban_list_20260926.py` | ✅ |
