...
## Owner 2026-09-24 晚：拿卡顺序与紧急资源（topic 分支，未进 fqyf7h）
...
- 身份锚点改指 `ee4c7ac`（这批最后一个改动 `src/shuabao` 的提交）。本分支并入 fqyf7h 用 merge commit 保留原 SHA；并入后 `src/shuabao` 再有提交就要重钉。
...
需要实机素材：羁绊栏里的蓝色 EX 卡（整屏帧 + 悬停详情），切成 `bond_bar/ex_card.png`；海盗 UR（毁灭战舰）同样。补模板时同步 `runtime_asset_manifest.json` 与门禁快照的资产数。
