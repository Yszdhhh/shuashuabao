...
本次实现已按 Owner 更正版更新：黑商仅在缺吞噬丹或木材低于 500 时作为快速获取路径；羁绊刷新每次面板最多 2 次，次数耗尽或木材不足时从当前可读候选按祝福、成长/经济、当前高级组、其它白名单、品质和最左顺序选卡；祝福不再由看板勾选且每局必拿。暂时隐藏仅用于调度器让路处理地面、背包或其它面板；无可读目标时保留面板并零输入等待。

默认高级卡组选序：海盗 → 大圣 → 刀刀 → 异火 → 海贼王 → 封神 → 修仙；亡灵仍为实验项且不默认勾选。低木材且技能积压时先点技能规则保持不变。发布门禁通过后仍需真机验证：刷新到上限后的选卡、刷新木材不足选卡、祝福强制必拿及上述组序。
