...
- 后续 bundle `solo_ingame_chain_20260920_200632_180078` 已正确进入 `STAGE_SELECT` 并写入 `direct_archaeology_arm`，但未发出 `SwitchToArchaeology`。根因是 `lobby/stage_archaeology_btn.png` 实为 `1-23` 关卡行残片；已从该 bundle 的 `f0007` 替换为真实“考古模式”按钮模板。新资产在该帧命中 `1.000`，在独立截图复核命中 `0.970`，中心均为 `(1363, 822)`。
...
截至本文写入，尚未获得模板修正后的新真机 bundle；不能据此宣称 Live PASS。
