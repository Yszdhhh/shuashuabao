...
- **资源**：扫荡券（扫荡关卡消耗）、**挑战券（开始游戏消耗；清空后切换考古模式）**、木材（羁绊抽卡）、宝物刷新次数、神器（Q/W/E 槽，固定 180s CD，17 种）。
- **自动化目标**：挂机刷图全流程：建房 → 房间开始 → 选关（滚轮找目标关）→ 英雄模式/普通模式 → 进局自动任务/四挑战/技能选卡 → 进化 → 神器释放 → 战后返回 → **挑战券清空自动进考古模式结束**。
...
- **券检测**：黄色挑战券（开始游戏按钮下方）剩余为 0 → 自动点考古模式 → 结束脚本
