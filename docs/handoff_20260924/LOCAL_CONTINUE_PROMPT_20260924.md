...
2. 如果 pytest 或 contract 有失败，现在汇总里会逐条打出 `FAILED <node id>` 和完整日志路径。遇到失败：
   ...
   - **允许重跑一次完整门禁**（Owner 2026-09-24 同意），但必须同时满足三条：
     - 失败的每个 node 所测的源码不在本轮改动范围内：用 `git log --oneline 1b0a1a8..HEAD -- <被测源码路径>` 检查，没有输出才算（测试文件本身不算，云端可能刚为它放宽过超时；例如启动器冒烟测试的被测源码是 `tools/launcher` 和 `src/shuabao/versioned_install.py`）；
     - 每个 node 单独跑 3 次全部通过；
     - 本轮还没用过这次重跑。
