...
- 蹭车长链实测（入口 13，测试版 `fc97d36`，20 局 + 进考古）：20 局跑满，16 胜 4 超时，无崩溃、无卡死，第 20 局后按设定退房、自建房、切入考古、正常退出。
  结果包 `G:\刷刷宝\captures\hitch_lobby_chain_20260925_011901_450471`（本机）；清单里的 `final_status=FAIL` 是 harness 的零输入预算和"每局回大厅"计数误报，不是卡死。
  逐局关卡：1 局 2-7、2 局 4-3、3 局 4-4（团本，房主团灭）、4 局 3-9、5–20 局 4-9（同一车头连带 16 局）。
- 实测暴露的 6 个问题全部修复，在 `fix/hitch-live-20260925`（Draft PR 指向 fqyf7h）：
  1. 传家宝"已挑战"误判：列表底排的红色 BOSS 角标落在 toast 检测带里，未点 Boss 就关弹窗（第 2、10、15 局超时）。改为只在本局点过 Boss 后才认 toast。**已修**。
  2. 时光之穴清单只到 54 莫阿姆，实机列表到 59（55 吞咽者布鲁 … 59 战争使者沙尔图拉）。从实机帧切 5 张模板、登记清单与资产清单。**已修**。
  3. 传家宝实机有第 21 个 Boss「界龟」；且"物理末卡"按像素底边比较，同一行 19/20 差 1 像素时选了 19。补 21 界龟模板，末卡改为"最后一行最右"。**已修**。
  4. 第 3 局团灭：半透明"失败"弹窗盖在时光之穴上，关闭按钮被连点 40 次，靠 300 秒上限退出。现有失败锚点认不出这种叠层弹窗。把「失败」横幅（f1229 原尺寸切图）加进 fail 场景，全局抢占接管并走左上角退出游戏。**已修**（全 5711 帧扫描：失败帧 1.0，其它最高 0.598）。
  5. 顶栏关卡"9"被认成"0"（3-9/4-9 记成 3-0/4-0）。**已修**，20 局重读全对。
  6. 每局关卡只打印到控制台，没进 trace。现写进 trace `s0`（`hitch_round_stage` 等）。**已修**。
  另：`55吞咽者布鲁` 以前被当成"未解锁哨兵"，现改为 `99未解锁哨兵`。
- 新需求（Owner 09-25）：**自动清理背包**。蹭车在局内点 HUD「存档」→「一键分解」→ 品质弹窗「是」；单人在选关页进入清理后继续。
  看板加开关，高级设置可调"每 N 局清理一次"（默认约 10 局）。分解不可逆：必须核对品质勾选（传说不得勾选）才点「是」，认不出就零输入。存档页、品质弹窗还缺实机截图模板。
- 单人第四轮：正式看板已存大圣 + 亡灵（`cards` 已写入 `user_settings.json`），入口 12 直接读取；门禁与快照刷新仍在本机进行。看板保存失败那次的根因是正式版 86266cb 配置队列首条 patch 被拒后锁死，重开看板即恢复。
- 背包清理进度（09-25 午）：蹭车入口 = 局内 HUD「存档」；单人入口 = 选关页顶栏「存档」页签，回程点「游戏大厅」页签。存档页默认停在「物品」页签（按钮是"一键回收"，不能点），要先切「装备」页签再点「一键分解」。
  模板已从只读实机截屏（`G:\刷刷宝\captures\backpack_clean_gt_20260925`）切好：`backpack/hud_cundang`、`quality_title`、`quality_yes`、`return_game`、`tab_cundang`、`tab_lobby`（对应帧 1.00，其它画面 ≤0.55）；复用竞品 `decompose`、`zhuangbei`（0.95）。竞品 `cundangInfo`、`yes` 对当前版本不匹配。
  勾选框按绿色像素读：只有 精良✓ 史诗✓ 传说☐ 才点「是」，否则点「否」退出。蹭车局内链路由 agy 实现中（staff-hub），单人入口随后接；看板开关与"每 N 局"需改 ui-v2 并重建。
- 分工（Owner 09-25）：偏执行的活（实现、切图、扫帧、跑测试、样板代码）经 staff-hub 派给外部 CLI（codex 做实现，opencode 做批量/样板，审查换另一个后端），Claude 只做拆任务、验收、纠偏。
- 门禁：本机第二次完整门禁仍红（4 条计时/OCR 测试，负载所致），按规矩停下；等 #46 合并后在机器空闲时跑一次，需 Owner 回"跑"。- 本机门禁：首跑除资产计数外 2 条吞噬丹旧测试与 6/10 规则冲突（已由 `90444813` 修正）；复跑出现 1 条授权测试在负载下偶发超时，单跑 3/3 通过。
