...
> 数据：155 独立 valid slots / 52 面板 / 21 套装进度槽 / 1 unknown 槽 / 0 unverified_layout 槽（不进分母）
...
| 门禁 | 阈值 | 实测 | 结论 |
|---|---:|---:|---|
| canonical_top1_accuracy | 0.95 | 0.9545 | PASS |
| lexicon_recall | 0.99 | 0.9545 | FAIL |
| mis_normalization | 0 | 0 | PASS |
| set_progress_accuracy | 0.95 | 0.9524 | PASS |
| panel3_cpu_p95 | 300.0 | 94.7 | PASS |
| single_cpu_p95 | 120.0 | 30.9 | PASS |
| rss_delta | 800.0 | 342.4 | PASS |
| offline_inference | 1.0 | 1.0 | PASS |
| model_hash_verified | manifest sha256+size per file (repo + staged) | True | PASS |
| repo_head_recorded | non-empty git rev-parse HEAD | 8facf06b04ad25b851ed65eac4b6164b31ff4836 | PASS |
...
|---|---:|---:|---:|---:|
| raw | 155 | 85.5% | 64.52% | 63.23% |
| pre | 155 | 86.14% | 67.1% | 65.81% |
...
- 词典内槽位（含别名覆盖，布局已验证）：154，其中严格词典内 154；词典外(unknown)：1；unverified_layout 剔除：0
- 规范名 Top-1（严格）：140/154 = 90.91%
- 规范名 Top-1（别名感知）：140/154 = 90.91%
- 词典召回（别名感知）：90.91%（154 槽）
- 技能词典召回（严格）：87.23%（47 槽）
- 误归一（词典外→配置名）：**0**（词典外正确置 None：1）
...
| rec1_ingame_boss_20260809 | 26 | 24 | 92.31% | 0 | 0 | 9 |
| rec3_260810 | 12 | 10 | 83.33% | 0 | 0 | 4 |
| rec5_260808 | 86 | 78 | 90.7% | 1 | 0 | 29 |
| rec7_short2_20260808 | 12 | 10 | 83.33% | 0 | 0 | 4 |

### 变体 pre

- 词典内槽位（含别名覆盖，布局已验证）：154，其中严格词典内 154；词典外(unknown)：1；unverified_layout 剔除：0
- 规范名 Top-1（严格）：147/154 = 95.45%
- 规范名 Top-1（别名感知）：147/154 = 95.45%
- 词典召回（别名感知）：95.45%（154 槽）
- 技能词典召回（严格）：91.49%（47 槽）
- 误归一（词典外→配置名）：**0**（词典外正确置 None：1）
...
| live_postgame_20260808 | 3 | 3 | 100.0% | 0 | 0 | 1 |
| reborn_wow_screens_20260806 | 9 | 9 | 100.0% | 0 | 0 | 3 |
| rec1_ingame_boss_20260809 | 26 | 26 | 100.0% | 0 | 0 | 9 |
| rec3_260810 | 12 | 12 | 100.0% | 0 | 0 | 4 |
| rec5_260808 | 86 | 80 | 93.02% | 1 | 0 | 29 |
| rec7_short2_20260808 | 12 | 11 | 91.67% | 0 | 0 | 4 |
...
|---|---|
| 模型加载时间 | 1.991s |
| 单槽 P50/P95（raw） | 20.9 / 41.8 ms |
| 单槽 P50/P95（pre） | 20.9 / 30.9 ms |
| 三槽连推 P50/P95（raw） | 62.3 / 89.0 ms |
| 三槽连推 P50/P95（pre） | 63.8 / 94.7 ms |
| RSS（import/加载后/稳态） | 75.0 / 447.6 / 789.9 MB |
| RSS 常驻增量 | 342.4 MB |
...
- 加载耗时：0.5s；推理耗时：0.044s；识别文本：'次级箭'
...
| `rec5_260808_f_045_skill_corrected_1_name.png (preprocessed)` | 奥术增幅α | '长箭 NEW奥术增幅' | '长箭奥术增幅' | None | skill |
| `rec5_260808_f_083_skill_corrected_1_name.png (preprocessed)` | 飓风 | 'W风NEW' | 'W风' | None | skill |
| `rec5_260808_f_086_bond_corrected_2_name.png (preprocessed)` | 吕岳 | '国' | '国' | '三国' | bond |
| `rec5_260808_f_104_bond_corrected_1_name.png (preprocessed)` | 海盗劫掠者 | '香院' | '香院' | None | bond |
| `rec7_short2_20260808_f_034_treasure_2star_2_name.png (preprocessed)` | 二星球 | '星球' | '星球' | None | 龙珠 |
...
- `fixtures\ocr_choices\bond\rec1_ingame_boss_20260809\t_036s_羁绊面板_slot1_name.png`：真值 '体魄' → OCR '体'（score=0.8898，lookup='体魄'）
- `fixtures\ocr_choices\bond\rec1_ingame_boss_20260809\t_036s_羁绊面板_slot2_name.png`：真值 '攻击提升' → OCR '击提升'（score=0.692，lookup='攻击提升'）
- `fixtures\ocr_choices\bond\rec1_ingame_boss_20260809\t_417s_转折点_slot1_name.png`：真值 '魔术' → OCR '厕术(0/2)'（score=0.6819，lookup='魔术'）
...
- `fixtures\ocr_choices\skill\rec3_260810\f_008_slot0_name.png`：真值 '箭矢连发' → OCR '失违发'（score=0.6291，lookup='箭矢连发'）
...
- `fixtures\ocr_choices\skill\rec5_260808\f_027_slot0_name.png`：真值 '飓风' → OCR '风NEW'（score=0.844，lookup='飓风'）
- `fixtures\ocr_choices\skill\rec5_260808\f_027_slot1_name.png`：真值 '陨石' → OCR ' 石 NEW'（score=0.8034，lookup='陨石'）
- `fixtures\ocr_choices\skill\rec5_260808\f_027_slot2_name.png`：真值 '电磁网' → OCR '电磁网NE'（score=0.4351，lookup='电磁网'）
- `fixtures\ocr_choices\skill\rec5_260808\f_043_slot2_name.png`：真值 '焦点爆破' → OCR ' 焦点慢础'（score=0.7243，lookup=None）
- `fixtures\ocr_choices\skill\rec5_260808\f_044_slot0_name.png`：真值 '飓风' → OCR ' 颶风 NEW'（score=0.7745，lookup='飓风'）
...
- `fixtures\ocr_choices\bond\rec5_260808\f_033_slot2_name.png`：真值 '箭矢扩散' → OCR '政本本前矢扩散'（score=0.5228，lookup=None）
- `fixtures\ocr_choices\bond\rec5_260808\f_034_slot2_name.png`：真值 '流星泪' → OCR ' '（score=0.3438，lookup=None）
- `fixtures\ocr_choices\bond\rec5_260808\f_035_slot0_name.png`：真值 '敏捷祝福' → OCR '教捷祝福'（score=0.7711，lookup='敏捷祝福'）
- `fixtures\ocr_choices\bond\rec5_260808\f_035_slot1_name.png`：真值 '杀敌多多' → OCR '希放多'（score=0.2387，lookup='杀敌多多'）
- `fixtures\ocr_choices\bond\rec5_260808\f_035_slot2_name.png`：真值 '法术增幅' → OCR '法术增幅主'（score=0.8883，lookup='法术增幅'）
- `fixtures\ocr_choices\bond\rec5_260808\f_039_slot0_name.png`：真值 '法术威力' → OCR '法术成力'（score=0.493，lookup='法术威力'）
- `fixtures\ocr_choices\bond\rec5_260808\f_041_slot0_name.png`：真值 '敏捷提升' → OCR '种教捷提升'（score=0.6396，lookup='敏捷提升'）
- `fixtures\ocr_choices\bond\rec5_260808\f_041_slot2_name.png`：真值 '气势' → OCR '气'（score=0.6447，lookup='气势'）
- `fixtures\ocr_choices\bond\rec5_260808\f_050_slot1_name.png`：真值 '金币猎人' → OCR '金而猎人'（score=0.8805，lookup='金币猎人'）
- `fixtures\ocr_choices\bond\rec5_260808\f_050_slot2_name.png`：真值 '魔法权杖' → OCR '直法权杖'（score=0.7086，lookup='魔法权杖'）
- `fixtures\ocr_choices\bond\rec5_260808\f_051_slot0_name.png`：真值 '魔爆能量' → OCR '爆能量'（score=0.6331，lookup='魔爆能量'）

## 7. 词典覆盖缺口（O1: 已纠正或标 unknown 的槽位）

unknown truth 槽共 1 个，独立计 unknown、不进准确率分母：
- `` ×1
