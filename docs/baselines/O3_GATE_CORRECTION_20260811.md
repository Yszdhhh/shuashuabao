...
**结论：O3 = CONDITIONAL_FAIL / BLOCKED。**
- skill_recall FAIL：4-5 个技能槽金边艺术字/徽章重叠/α/β 后缀（根因具体），修复中（SKILL-FIX），修复后重评（不得改分母/删失败样本/降阈值）。
- 960×540 与断线素材 BLOCKED：无用户实机素材前，OCR 生产接入（O4 接线）、G0 后段、长期无人值守宣称一律冻结。
