...
| panel3_cpu_p95 | 300.0 | 102.4 | PASS |
| single_cpu_p95 | 120.0 | 49.5 | PASS |
| rss_delta | 800.0 | 223.8 | PASS |
...
- 词典内槽位（含别名覆盖）：276，词典外：34
- 规范名 Top-1（严格）：4/272 = 1.47%
- 规范名 Top-1（别名感知）：4/276 = 1.45%
- 词典召回（别名感知）：1.45%（276 槽）
- 技能词典召回（严格）：4.44%（90 槽）
- 误归一（词典外→配置名）：**0**（词典外正确置 None：34）
...
|---|---|
| 模型加载时间 | 3.113s |
| 单槽 P50/P95（raw） | 22.8 / 49.5 ms |
| 单槽 P50/P95（pre） | 23.5 / 47.0 ms |
| 三槽连推 P50/P95（raw） | 66.7 / 102.4 ms |
| 三槽连推 P50/P95（pre） | 71.7 / 107.8 ms |
| RSS（import/加载后/稳态） | 21.3 / 456.9 / 680.7 MB |
| RSS 常驻增量 | 223.8 MB |
| 模型目录（仓库） | C:\Users\10639\Desktop\🎮 影音游戏\GameScript-Local\models\ocr\PP-OCRv5_mobile_rec_infer |
| 模型目录（ASCII 暂存） | C:\tmp\ocr_stage_mobile4\PP-OCRv5_mobile_rec_infer |
| 采样量 | 单槽 310、三槽 104 |

## 5. 断网/离线校验

- 结果：PASS — socket 出站连接全阻断下完成模型加载+推理
- 加载耗时：0.494s；推理耗时：0.049s；识别文本：'福7'
...
- `元素之力` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `利刃` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `利刃海盗` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `力量之源` ×6：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `吕岳` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `奥术激光` ×4：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `射手姿态` ×4：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `恢复神符` ×4：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `海盗劫掠者` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
- `白赚海盗` ×2：不在 config/choice_lexicon.json，该槽生产 Top-1 必然失败（词典补录后可恢复）。
