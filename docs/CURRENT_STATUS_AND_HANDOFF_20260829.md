...
`G:\下载\1.5.1.zip\Images\cards` 不是 OCR 训练集，是 ~44×24 的 `matchTemplate` 标题字形。live F 以前读完 OCR 就返回，**从未**用这些图填空槽。另外短码标签和像素对不上：`jj`=经济、`tz`=挑战、`fs`=法神、`gushou`=固守、`yihuo`=异火、`mfs`=秘法师；`法术/急速/魔能` 在包根目录 `fashu/jisu/moneng`。卡住的 4 选（三国/刀刀/挑战2/3/暴击）上官方 `tz`/`baoji` 标题分 0.90/0.96，OCR 空白也能点挑战。桌面 exe 仍是旧包，要实机验证需重新打包。
